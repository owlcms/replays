...
// ExtractDefaultMulticastConfig extracts the embedded multicast.toml if none exists.
...
// LoadMulticastConfig loads multicast settings from the given path.
func LoadMulticastConfig(multicastPath string) (config.MulticastSettings, error) {
	var file multicastFile
	if _, err := toml.DecodeFile(multicastPath, &file); err != nil {
		return config.MulticastSettings{}, err
	}
...
func UpdateMulticastMappingFile(multicastPath string, settings config.MulticastSettings) error {
	...
	newSection := []string{
		...
		"# 3) default_cameras.toml platform sections (legacy fallback)",
		"# 4) legacy camera sections in config.toml ([windows*], [linux*], etc.)",
		"#",
